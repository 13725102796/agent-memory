...
from memory_agent.types import MemoryRecord, MemoryType, PackSearchResult, SearchResult
...
class MemorySearcher:
    ...
    def search(
        ...
        merged = self._merge_scores(vector_hits, bm25_hits, user_id, type_filter)
        _t_merge = time.time()
    ...
    def _merge_scores(
        ...
        for mid in all_ids:
            ...
            # 时间衰减：越旧的记忆分数越低（下限 0.5）
            decay = time_decay_factor(rec.updated_at)
            final *= decay
            ...
            # 按类型过滤
            if type_filter and rec.memory_type not in type_filter:
                continue

            results.append(SearchResult(
                id=mid, content=rec.content, score=final,
                tier=rec.tier, pack_id=rec.pack_id,
                memory_type=rec.memory_type, name=rec.name,
    ...
    def _aggregate_packs(
        ...
        # 计算权重：hit_count × avg_score，过滤低权重噪声
        ...
        for pack_id, scores in pack_votes.items():
            ...
            if weight < 0.1:
                continue  # 过滤噪声：权重过低的 pack 不参与排名
...
def _min_max_normalize(scores: dict[str, float]) -> dict[str, float]:
    """Min-Max 归一化到 [0, 1]。
    当所有分数相同时保留原始值（而非全部设为 1.0），避免消除区分度。
    """
    if not scores:
        return {}
